<template>
	<NuxtLayout name="default" nav="개인정보처리방침">
...
const callApi = () => {
	terms
		.getPrivacyList(searchForm.value)
		.then(({ data }) => {
...
const handlerClickSearchButton = () => {
	...
	router.push(`/terms-manage/privacy${getQueryString(searchForm.value)}`);
};
...
const handlerClickTableColumn = seq => {
	router.push(`/terms-manage/privacy/write/${seq}`);
};
</script>
